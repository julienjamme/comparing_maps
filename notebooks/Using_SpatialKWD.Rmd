...
```{r}
r_meuse <- rasterFromXYZ(meuse.grid[, c("x","y","dist")])
nrow(r_meuse) <- ncol(r_meuse)
r_meuse_flip1 <- flip(r_meuse, direction = 1)
r_meuse_flip2 <- flip(r_meuse, direction = 2)

s_meuse <- stack(r_meuse, r_meuse_flip1, r_meuse_flip2)
plot(s_meuse)

coordinates <- coordinates(s_meuse)
weights <- cbind(
  values(s_meuse$dist.1),
  values(s_meuse$dist.2),
  values(s_meuse$dist.3)
)
matrix_to_maps(coordinates, weights) |> plot()
```

```{r}
compareOneToMany(coordinates, weights, recode = TRUE)
```
